		...
		$sql = "SELECT count(u.id) as total "
            ...
            //print_r($_POST);
                ...
                $sql = "SELECT count(u.id) as total "
                                . "FROM usuarios u JOIN datos_personales dp ON(u.id = dp.id) JOIN curriculo c ON (c.id = dp.id) JOIN estatus_postulacion ep ON (ep.curriculo = c.id) "
                                . "WHERE u.eliminado = '0' AND nivel = 'Postulante' "
                                . "AND (profesion LIKE lower('%$busqueda%') or nombre LIKE lower('%$busqueda%') or apellido LIKE lower('%$busqueda%')"
                                . " or (cedula LIKE lower('%$busqueda%')  AND u.eliminado = '0' AND nivel = 'Postulante')"
                                . " or (categoria LIKE lower('%$busqueda%')  AND u.eliminado = '0' AND nivel = 'Postulante') )"
                                . " or (lower(ep.estatus) = lower('$busqueda') AND u.eliminado = '0' AND nivel = 'Postulante') "
                                . "ORDER BY ep.estatus, nombre, apellido";
        }
		$maximo = 10;
		if(!empty($_GET['pag'])){
...
<body data-spy="scroll" data-target=".bs-docs-sidebar">
       	    ...
       	    <div class="row-fluid">
    	        ...
    	        <div class="span6" align="right">
                    ...
                    <form name="form1" method="post" action="" class="form-inline">
                        ...
                        <button name='btn-categoria' class="btn btn-default ">Buscar</button>
                    </form>
                    <form name="form1" method="post" action="" class="form-inline">
                ...
                <div  class="span6" align="right" id="filtro-postulante">
                    <form method="POST" class="form-inline" name="form-fitro">
                        <select class="span3" name="edad_desde" id="edad-desde">
                            <option value="">Edad Desde</option>
                            <?php for($edad=18;$edad<81;$edad++): ?>
                                <option value="<?php echo $edad ?>"><?php echo $edad; ?></option>
                            <?php endfor;?>
                        </select>

                        <select class="span3"  name="edad_hasta" id="edad-hasta">
                            <option value="">Edad Hasta</option>
                            <?php for($edad=18;$edad<81;$edad++): ?>
                                <option value="<?php echo $edad ?>"><?php echo $edad; ?></option>
                            <?php endfor;?>
                        </select>
                        <input type="text" name="profesion_filtro" placeholder="Escriba la profesión">
                        <button name='btn-filtro' class="btn btn-default ">Buscar</button>
                    </form>
                </div>
            </div>
        </td>
	...
	<table class="table table-bordered table table-hover">
      ...
      <?php
		...
		if(empty($_POST['busqueda'])){
			$sql="SELECT TIMESTAMPDIFF(YEAR, fecha_natal, CURDATE()) as edad, fecha_estatus, cargo_postulado, fecha_postulacion, descripcion, dp.id, cedula, dp.nombre, dp.apellido, CONCAT(dp.nombre, ' ', dp.apellido) as nombre_completo, profesion, categoria, cargo_postulado, ep.estatus as estatus_p, recomendador "
                                . "FROM usuarios u JOIN datos_personales dp ON(u.id = dp.id) JOIN curriculo c ON (c.id = dp.id) JOIN estatus_postulacion ep ON (ep.curriculo = c.id) "
                                . "WHERE u.eliminado = '0' AND nivel = 'Postulante' "
			...
			$sql="SELECT TIMESTAMPDIFF(YEAR, fecha_natal, CURDATE()) as edad, fecha_estatus, cargo_postulado, fecha_postulacion, descripcion, dp.id, cedula, dp.nombre, dp.apellido, CONCAT(dp.nombre, ' ', dp.apellido) as nombre_completo, profesion, categoria, cargo_postulado, ep.estatus as estatus_p, recomendador "
                ...
                if(isset($_POST["btn-filtro"])) {
                    $profesionFiltro = empty($_POST["profesion_filtro"])?" ":limpiarCampo($_POST["profesion_filtro"]);
                    $edadDesde = limpiarCampo($_POST["edad_desde"]);
                    $edadHasta = limpiarCampo($_POST["edad_hasta"]);
                    if(empty($_POST["profesion_filtro"])) {
                        $sql="SELECT TIMESTAMPDIFF(YEAR, fecha_natal, CURDATE()) as edad, fecha_estatus, cargo_postulado, fecha_postulacion, descripcion, dp.id, cedula, dp.nombre, dp.apellido, CONCAT(dp.nombre, ' ', dp.apellido) as nombre_completo, profesion, categoria, cargo_postulado, ep.estatus as estatus_p, recomendador "
                            . "FROM usuarios u JOIN datos_personales dp ON(u.id = dp.id) JOIN curriculo c ON (c.id = dp.id) JOIN estatus_postulacion ep ON (ep.curriculo = c.id) "
                            . "WHERE u.eliminado = '0' AND nivel = 'Postulante' "
                            . " "
                            . "AND TIMESTAMPDIFF(YEAR, fecha_natal, CURDATE()) BETWEEN '$edadDesde' AND '$edadHasta'  "                            
                            . "ORDER BY ep.estatus, nombre, apellido LIMIT $inicio, $maximo";
                    } else {
                        $sql="SELECT TIMESTAMPDIFF(YEAR, fecha_natal, CURDATE()) as edad, fecha_estatus, cargo_postulado, fecha_postulacion, descripcion, dp.id, cedula, dp.nombre, dp.apellido, CONCAT(dp.nombre, ' ', dp.apellido) as nombre_completo, profesion, categoria, cargo_postulado, ep.estatus as estatus_p, recomendador "
                            . "FROM usuarios u JOIN datos_personales dp ON(u.id = dp.id) JOIN curriculo c ON (c.id = dp.id) JOIN estatus_postulacion ep ON (ep.curriculo = c.id) "
                            . "WHERE u.eliminado = '0' AND nivel = 'Postulante' "
                            . " "
                            . "AND (profesion LIKE lower('%$profesionFiltro%') AND TIMESTAMPDIFF(YEAR, fecha_natal, CURDATE()) BETWEEN '$edadDesde' AND '$edadHasta' ) "                            
                            . "ORDER BY ep.estatus, nombre, apellido LIMIT $inicio, $maximo";                        
                    }
                }
      ...
      <tr>
        ...
        <td title="<?php echo $nombreCompleto; ?><?php echo " (".$dato['edad'].")" ?>">
            <a href='detalle-postulante.php?id=<?php echo $dato['id'] ?>'>
                <?php echo $nombreCompleto; ?></a> <?php echo "(".$dato['edad'].")" ?></td>
...
<?php
/**
 * INICIO: CAmbiar estatus del postulante
 */
