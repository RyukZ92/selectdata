...
if (isset($_POST["guardar"])) {
    ...
    $sizeAllowImg = 300;
    if ($tamanio_foto > $sizeAllowImg OR $_FILES[('foto')]['error'] == 1 AND strlen($_FILES[('foto')]['name']) > 0)
        $msj = notificacion("El archivo no debe ser mayor a <b>".$sizeAllowImg."kb</b>", "danger");
    else if (strtolower($tipo_foto[1]) != 'png'
                && strtolower($tipo_foto[1]) !='jpeg'
                && strtolower($tipo_foto[1]) != 'jpg'
                && strtolower($tipo_foto[1]) !='gif'
            && strlen($_FILES[('foto')]['name']) > 0)
        $msj = notificacion("Solo se aceptan fortmatos <b>JPEG, JPG, GIF Y PNG</b>", "danger");
    else {
